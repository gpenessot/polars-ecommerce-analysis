...
results_dir = "/home/gael/Documents/Github/polars-ecommerce-analysis/data/processed/report_20250119_004442/results"
...
total_revenue = f"{global_kpis['total_revenue']:,.2f}"
total_orders = f"{global_kpis['total_orders']:,d}"
average_order_value = f"{global_kpis['average_order_value']:,.2f}"
total_customers = global_kpis['total_customers']
total_products = global_kpis['total_products']
average_items_per_order = f"{global_kpis['average_items_per_order']:.1f}"

# Préparation des données pour les recommandations
top_price_category = price_analysis[0]['PriceCategory']
bottom_price_category = price_analysis[-1]['PriceCategory']
champions_count = customer_metrics.group_by('RFM_Segment').agg(pl.count('CustomerID')).sort('CustomerID', descending=True)[0, 'CustomerID']
```
...
- Nombre total de commandes : {total_orders}
...
1. Performance Générale
   ...
   - {total_products} produits différents vendus
   - Moyenne de {average_items_per_order} articles par commande
...
1. **Optimisation de la gamme de prix**
   - Les produits {top_price_category} génèrent le plus de revenus
   - Opportunité d'expansion dans la catégorie {bottom_price_category}

2. **Gestion de la clientèle**
   - Focus sur la rétention des "Champions" ({champions_count} clients)
